...
//Recibe nombre del archivo, lo carga y retorna como string
...
vector<string> SplitMessage(string _message, string _separator){
    ...
    //boost::split(result, _message, boost::is_any_of(_separator));
		char temp_messagge[_message.length() + 1];
		strcpy(temp_messagge, _message.c_str());

		char* temp_al;
		temp_al = strtok(temp_messagge, _separator.c_str());
		while(temp_al != NULL){
			temp_result = temp_al; result.push_back(temp_result);
			temp_al = strtok(NULL, _separator.c_str());
		}
