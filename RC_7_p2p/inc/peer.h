...
void TPeer::Init(){
    ...
    int nr = bind(m_peer_server_sock, (struct sockaddr *) &m_peer_server_addr, sizeof(sockaddr_in)); //Assigning name to the socket
    if(nr < 0){
...
void TPeer::SSave(string _key){
    ...
    for(unsigned i=0; i<block_pair.size(); i++){
        std::cout<<m_file_complete[block_pair[i].second];
...
void TPeer::CConnectAndSend(TPeerInfo _machine, string _message, string _type){

    int peer_clt_sock = socket(AF_INET, SOCK_STREAM, 0); //Creando socket
      if(peer_clt_sock < 0){
          perror("Can not Create Socket!");
          exit(0);
      }
    //Configurando socket interno de recepcion  peer_clt_addr
    ...
    while(m_state){
        ...
        switch(cmmd){
            ...
            case 'u':{
                ...
                tfile = ReadFile("upload/"+message); //From Utils.h
                std::cout<<"Subiendo mensaje: "<<std::endl;
            ...
            default:
                if(cmmd >= '1' and cmmd <= '4'){
                    ...
                    tfile = ReadFile("upload/"+files_name[nfile]); //From Utils.h
...
//Nombre del archivo, chunks por peers
...
void TPeer::CUpload(string _file_key ,string _file_body){ //Nombre del archivo,  cuerpo del archivo.
    vector<string> nchunk = SplitText(_file_body, m_chunk_size); //From protocol.h

    for(unsigned int i = 0; i < nchunk.size(); i++){//Segmentando
      std::cout<<" Chunk "<<i<<" --> [ "<<nchunk[i]<<" ]"<<std::endl;
    }

/*    if(m_neighboring_peers.size() != 0){
      for(unsigned int i = 0; i < nchunk.size() ; i++){
        int select_neighbord = rand() % m_neighboring_peers.size();// A cual de todos vamos a enviar el chunk
        std::cout<<"Sending chunk "<<i<<" to:";
        PrintPeer(m_neighboring_peers[select_neighbord]);
        std::cout<<std::endl;

        CConnectAndSend(m_neighboring_peers[select_neighbord], nchunk[i], "U");
      }
    }
*/

    if(m_neighboring_peers.size() != 0){ //Distribución de Peers
        int nbpp = (int)nchunk.size()/((int)m_neighboring_peers.size()+1);
        int mbpp = (int)nchunk.size()%((int)m_neighboring_peers.size()+1);

        nbpp += (mbpp == 0)? 0: 1; //So, each peers receives.
        ...
        //int k = 0;
        int k = (rand() % m_neighboring_peers.size()) + 1;
        string ablock;
        ...
        for(unsigned int i = 0; i < nchunk.size(); i++){
            ...
            //k++;
            k = (rand() % m_neighboring_peers.size()) + 1;
            std::this_thread::sleep_for(std::chrono::milliseconds(5000));
        }
    }
    else{
...
void TPeer::SListening(){
    ...
    while(m_state){
        ...
        if(ConnectSock < 0)
            ...
            if(command[0] != 'K')
              std::cout<<"Recibido: "<<"[ "<< command << " ]"<<std::endl;
